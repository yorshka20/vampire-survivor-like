<script lang="ts">
  ...
  // GitHub link, pointing straight at this harness's source file (not the repo
  // root) so a reader of the rendering test lands on the code behind it.
  const repoUrl = import.meta.env.VITE_REPO_URL;
  const sourceUrl = `${repoUrl}/blob/main/packages/simulator/src/createRenderingTest.ts`;
...
<!-- Jump to this harness's source on GitHub. Always visible (even pre-start),
     bottom-right, above the controls panel so it stays clickable. -->
<a href={sourceUrl} target="_blank" rel="noopener noreferrer" class="github-button">
...
<style>
  ...
  .github-button {
    ...
    border-radius: 5px;
    cursor: pointer;
    /* Above the controls panel (z-index 1100) so it stays clickable. */
    z-index: 1200;
