<script lang="ts">
  ...
  // GitHub link, pointing straight at this simulator's source file (not the repo
  // root) so a reader lands on the code behind it.
  const repoUrl = import.meta.env.VITE_REPO_URL;
  const sourceUrl = `${repoUrl}/blob/main/packages/simulator/src/createSimulator.ts`;
...
<!-- Jump to this simulator's source on GitHub. Always visible (even pre-start),
     bottom-right, just left of the Rendering Test link. -->
...
<style>
  ...
  .github-button {
    position: fixed;
    /* Sit just left of the Rendering Test link (which is pinned at right: 10px). */
    right: 180px;
    bottom: 10px;
    color: white;
    font-family: monospace;
